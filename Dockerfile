...
RUN apt-get update \
    ...
    && curl -L -O https://github.com/xmrig/xmrig-nvidia/archive/v2.4.5.tar.gz \
    && tar -xvf v2.4.5.tar.gz \
    && cd xmrig-nvidia-2.4.5 \
    && mkdir build \
    && cd build \
    && cmake .. -DCUDA_TOOLKIT_ROOT_DIR=/usr/local/cuda-9.1 -DWITH_HTTPD=OFF \
    && make \
    && cd ../.. \
    && mv xmrig-nvidia-2.4.5/build/xmrig-nvidia /usr/local/bin/xmrig-nvidia \
    && chmod a+x /usr/local/bin/xmrig-nvidia \
    && rm -r xmrig-nvidia-2.4.5 \
