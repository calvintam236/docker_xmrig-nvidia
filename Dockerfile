...
RUN apt-get update \
    ...
    && curl -L -O https://github.com/xmrig/xmrig-nvidia/archive/v2.14.1.tar.gz \
    && tar -xvf v2.14.1.tar.gz \
    && rm v2.14.1.tar.gz \
    && cd xmrig-nvidia-2.14.1 \
    && mkdir build \
    && cd build \
    && cmake .. -DCUDA_TOOLKIT_ROOT_DIR=/usr/local/cuda-10 \
    && make \
    && cd ../.. \
    && mv xmrig-nvidia-2.14.1/build/xmrig-nvidia /usr/local/bin/xmrig-nvidia \
    && chmod a+x /usr/local/bin/xmrig-nvidia \
    && rm -r xmrig-nvidia-2.14.1 \
